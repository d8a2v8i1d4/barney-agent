...
**正確做法**：同日二執檢查 `[TODAY].json` 是否已存在——
- 已存在且有內容 → **保留不動**，只更新 `lastrun.txt`（heartbeat 追加二執記錄），再走第九步 push
- 不存在 → 才走原本邏輯（情況 A 寫活動、情況 B 寫 `[]`）

另外，同日二執遇到「情況 B + 週日」時，若當天首執已寄過完整日報+週報 Top 5，**不要再寄一份週報**——會變成信箱重複轟炸，比起「靜默」更擾人。
